...
def build_id_to_token_vocab(token_to_id):
    ...
    for tok in token_to_id:
        id_to_token[token_to_id[tok]] = tok
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for tok in sentence.split():
        if tok in token_to_id:
            encode_sentence.append(token_to_id[tok])
        else:
            encode_sentence.append(token_to_id[unk_token])
...
def build_position_index_column(max_len):
    ...
    return torch.reshape(i, (max_len, 1))
    #return torch.tensor([[i] for i in range(max_len)], dtype=torch.float32)
...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    res = torch.zeros((max_len, d_model), dtype=torch.float32)

    div_term = compute_positional_div_term(d_model)
    position = build_position_index_column(max_len)

    res = fill_even_indices_with_sin(res, position, div_term)
    res = fill_odd_indices_with_cos(res, position, div_term)
